...
def naiveSoftmaxLossAndGradient(
    ...
    ### to integer overflow.
    softmax_ = softmax(np.dot(outsideVectors, centerWordVec))
    loss = -np.log(softmax_[outsideWordIdx])
    temp = np.zeros(softmax_.shape)
    temp[outsideWordIdx] = 1
    gradCenterVec = np.sum(softmax_.reshape((-1, 1)) * outsideVectors, axis=0) - outsideVectors[outsideWordIdx]
    gradOutsideVecs = np.tile(centerWordVec, (outsideVectors.shape[0], 1)) * (softmax_ - temp)[:, np.newaxis]
    ### END YOUR CODE
...
def negSamplingLossAndGradient(
    ...
    OutWordVec = outsideVectors[outsideWordIdx]
    OutWordSig = sigmoid(np.dot(OutWordVec, centerWordVec))
    NegSamples = outsideVectors[negSampleWordIndices]
    loss = -np.log(OutWordSig) - np.sum(np.log(sigmoid(np.dot(-NegSamples, centerWordVec))), axis=0)
    gradCenterVec = -OutWordVec * (1 - OutWordSig) + np.sum(NegSamples * (1 - sigmoid(np.dot(-NegSamples, centerWordVec))).reshape(-1,1), axis=0)
    gradOutsideVecs = np.zeros(outsideVectors.shape)
    for idx, neg_idx in enumerate(negSampleWordIndices):
        gradOutsideVecs[neg_idx] += centerWordVec * (1 - sigmoid(np.dot(-NegSamples[idx], centerWordVec)))
    gradOutsideVecs[outsideWordIdx] = -centerWordVec * (1 - sigmoid((np.dot(OutWordVec, centerWordVec))))
    ### END YOUR CODE
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for word in outsideWords:
        ...
        loss += temp1
        gradCenterVecs[word2Ind[currentCenterWord]] += temp2
        gradOutsideVectors += temp3
    ### END YOUR CODE
