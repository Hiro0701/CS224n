...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        self.embed_to_hidden_weight = nn.Parameter(nn.init.xavier_uniform(torch.Tensor(self.n_features * self.embed_size, self.hidden_size)))
        self.embed_to_hidden_bias = nn.Parameter(nn.init.uniform(torch.Tensor(1, self.hidden_size)))
        self.dropout = nn.Dropout(self.dropout_prob)
        self.hidden_to_logits_weight = nn.Parameter(nn.init.xavier_uniform(torch.Tensor(self.hidden_size, self.n_classes)))
        self.hidden_to_logits_bias = nn.Parameter(nn.init.uniform(torch.Tensor(1, self.n_classes)))
    ...
    def embedding_lookup(self, w):
        ...
        x = [ torch.index_select(self.embeddings, 0, w[i]) for i in range(w.shape[0]) ]
        x = [ torch.tensor(tensors).view(-1) for tensors in x ]
        x = torch.stack(x)
