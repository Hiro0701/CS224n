...
class NMT(nn.Module):
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        Y = F.embedding(target_padded, self.model_embeddings.source.weight)
        splitted_Y = torch.vsplit(Y, Y.size(0))
        for Y_t in splitted_Y:
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        e_t = torch.squeeze(torch.bmm(torch.unsqueeze(dec_state[0], dim=1), torch.transpose(enc_hiddens_proj, 1, 2)))
        ...
        a_t = torch.sum(alpha_t.unsqueeze(2).expand(-1, -1, 2*self.hidden_size) * enc_hiddens, dim=1)
